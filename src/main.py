...
def read_root(request: Request) -> dict[str, dict[str, str | dict[str, str]] | str]:
    ...
    4. docker volume access at /app/volume_cache
    5. AWS S3 access
    ...
    try:
        with Path.open(Path("/app/volume_cache/test.txt"), "w") as f:
            _ = f.write("success-" + formatted_time)
        with Path.open(Path("/app/volume_cache/test.txt")) as f:
    ...
    with Path.open(Path("/app/volume_cache/test.txt"), "rb") as f:
        file_content = f.read()
        s3_test_put_file_id = (
