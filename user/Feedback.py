...
def get_agent_by_id(request: Request, thread_id: str, db: Annotated[Session, Depends(get_db)]):
    """
    ...
    try:
        ...
        if (message_id and rating in [0, 1]) or (not message_id and rating > 0 and rating <= 5):
            db.add(
                UserFeedback(
